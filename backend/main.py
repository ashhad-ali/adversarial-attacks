...
def load_image(file_bytes: bytes):
    ...
    tensor = transform(image).unsqueeze(0)  # [1, 3, 224, 224] in [0, 1] range
    return tensor


def predict(tensor):
    """Run a forward pass and return (class_index, class_label)."""
    # Manually normalize the [0, 1] tensor just in time for the model
...
def tensor_to_base64(tensor):
    ...
    # The tensor is already in [0, 1], so we just clamp to be safe
    img = img.clamp(0, 1)
...
async def attack_deepfool(
    ...
    attack = torchattacks.DeepFool(model, steps=150, overshoot=1)
    attack.set_normalization_used(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
async def attack_cw(
    ...
    steps: int = Form(100), # Lowered to 100 to prevent HTTP timeouts locally/on HuggingFace
):
    tensor = load_image(await file.read())
    original_idx, original_label = predict(tensor)

    attack = torchattacks.CW(model, c=5, kappa=confidence, steps=steps, lr=0.05)
    attack.set_normalization_used(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
